%% Question 1 part a. Analysis of thin plate using CPT elements
clc                                             %clears screeen
...
t = 2.5E-03;                                    %thickness of plate
lx = 0.254;                                     %length of element in x-direction
ly = 0.254;                                     %ength of element in y-direction
P = 445;                                        %Point load
...
for i=1:ne                                      %Membrane stiffness
    ...
    for k = 1:p
        for l = 1:q
            ...
            for a=1:nne
                ...
                Bbe(1,5*a-1) = Bxx(1,3*a-1);
                Bbe(1,5*a) = Bxx(1,3*a);
                Bbe(2,5*a-2) = Byy(1,3*a-2);
                Bbe(2,5*a-1) = Byy(1,3*a-1);
                Bbe(2,5*a) = Byy(1,3*a);
                Bbe(3,5*a-2) = 2*Bxy(1,3*a-2);
                Bbe(3,5*a-1) = 2*Bxy(1,3*a-1);
                Bbe(3,5*a) = 2*Bxy(1,3*a);
            end
    ...
    Ke=kme+kbe;
   % Fe(23,1) = t*                                               le/2*[0; 0; Tx(i); 0; Tx(i); 0; 0; 0]; 
    for j=1:dofe
        for k=1:dofe
...
%% Beam element start
BeamConn=[7,8,37,38; 17,18,27,28];
I=0.05*0.3^3/12;
le=0.254;
...
for i = 1:2
    ...
    for j = 1:4
        for k = 1:4
            KG(BeamConn(i,j), BeamConn(i,k)) = KG(BeamConn(i,j), BeamConn(i,k)) + KE(j,k);
        end
        %FGU(BeamConn(i,j),1) = FGU(BeamConn(i,j),1) + FE(j,1);
    end
end
...
%% Output
disp('The maximum vertical deflection of the plate is (in m)')
disp(abs(UG(3)))
